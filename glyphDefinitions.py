...
drivesFolder = f'{baseFolder}drives/'
...
drivesCount = 26
drivesPrefix = f'{drivesFolder}letters/Drive'
...
driveSVGs = [f'{drivesFolder}Drive.svg'] + \
    [f'{drivesFolder}Drive{icon}.svg' for icon in iconList]
...
driveLettersSVGs = []
for i in range(drivesCount):
    letter = chr(ord('A') + i)
    driveLettersSVGs += [f'{drivesPrefix}{letter}.svg']
